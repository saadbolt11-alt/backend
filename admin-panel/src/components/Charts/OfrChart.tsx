...
import { Info, ExternalLink, MoveHorizontal as MoreHorizontal } from 'lucide-react';
...
export default function OFRChart({ chartData, hierarchyChartData, timeRange = 'day' }: OFRChartProps) {
  ...
  const data = useMemo(() => {
    if (chartData?.chartData) {
      ...
      return chartData.chartData.map(point => ({
        time: point.timestamp, // Use raw timestamp for dynamic formatting
        line: point.ofr || 0,
        ...
        <div className="flex items-center justify-center h-80">
          <div className="text-center">
            <p className={`text-lg font-medium ${
              ...
              No OFR data available
            </p>
            <p className={`text-sm ${
              theme === 'dark' ? 'text-gray-500' : 'text-gray-500'
            }`}>
              This device type may not support oil flow rate measurements
            </p>
          </div>
        </div>
      ) : (
